...
export class DecodeApiClient {
  ...
  async getUserById(
    ...
    try {
      ...
      const response = await firstValueFrom(
        this.httpService.get<{ data: UserProfile }>(`${this.decodeApiUrl}/users/profile/${userId}`, {
          headers: {
  ...
  private async getAccessTokenFromSession(sessionId: string): Promise<string | null> {
