...
export class GatewayService {
  ...
  async proxyRequest(
    ...
    for (let attempt = 1; attempt <= serviceConfig.retries; attempt++) {
      try {
        ...
        this.logger.log(
          `✅ Success response from ${serviceName}: ${response.status}`,
        );
        return {
          data: response.data,
        ...
        if (error.response) {
          this.logger.log(
            `✅ Forwarding response from ${serviceName}: ${error.response.status}`,
          );
          return {
        ...
        if (attempt < serviceConfig.retries) {
          ...
          this.logger.warn(
            `⚠️ Attempt ${attempt} failed for ${serviceName}, retrying in ${delay}ms...`,
          );
    ...
    // All retries failed
    this.logger.error(
      `❌ Service ${serviceName} failed after ${serviceConfig.retries} attempts:`,
      lastError.message,
    );
  ...
  private async checkServiceHealth(serviceName: string): Promise<boolean> {
    ...
    if (
      ...
      now - lastCheck < this.healthCheckInterval
    ) {
    ...
    try {
      ...
      if (
        ...
        this.lastHealthCheck[serviceName] = now;
        this.logger.debug(
          `✅ Service ${serviceName} is up (got ${error.response.status})`,
        );
        return true;
      }

      this.logger.warn(
        `⚠️ Service ${serviceName} health check failed: ${error.message}`,
      );
      this.serviceStatus[serviceName] = false;
      this.lastHealthCheck[serviceName] = now;
      return false;
  ...
  async refreshServiceStatus(): Promise<void> {
    ...
    const healthChecks = Object.keys(this.services).map(async (serviceName) => {
      ...
      this.logger.log(
        `${isHealthy ? "✅" : "❌"} ${serviceName}: ${isHealthy ? "UP" : "DOWN"}`,
      );
    });
