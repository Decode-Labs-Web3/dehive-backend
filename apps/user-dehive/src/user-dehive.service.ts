...
export class UserDehiveService {
  constructor(
    @InjectModel('UserDehive') private readonly userDehiveModel: Model<UserDehive>,
  ...
  async checkDecodeAuthToken(decode_auth_token: string) {
    try {
      ...
      console.log('user-dehive service checkDecodeAuthToken', `${apiUrl}/auth/verify-decode-auth-token`);
      console.log('user-dehive service checkDecodeAuthToken', decode_auth_token);
      const response = await axios.post(`${apiUrl}/auth/verify-decode-auth-token`, {}, {
        headers: {
  ...
  async authCheckCallback(sso_token: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.post(`${apiUrl}/auth/get-refresh-token-by-sso-token`, {
      sso_token: sso_token,
    });
    console.log('user-dehive service authCheckCallback', response.data);
    const call_back_response = response.data;
    if (!call_back_response.refresh_token || call_back_response.statusCode !== 200) {
      return {
        statusCode: 401,
        message: 'User not authenticated',
      };
    }
    const dehive_access_token = this.jwtService.sign({refresh_token: call_back_response.refresh_token}, {
      expiresIn: '1h',
    });
  ...
  async refreshSession(refresh_token: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.post(`${apiUrl}/auth/refresh-session`, {}, {
      headers: {
  ...
  async sendFriendRequest(auth: string, to_user_id: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.post(`${apiUrl}/relationship/friends/requests`, {
  ...
  async removeFriendRequest(auth: string, to_user_id: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.delete(`${apiUrl}/relationship/friends/requests`, {
      headers: { Authorization: auth },
  ...
  async acceptFriendRequest(auth: string, requester: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.post(`${apiUrl}/relationship/friends/request-accept`, {
      requester: requester
    }, {
      headers: { Authorization: auth }
    });
  ...
  async rejectFriendRequest(auth: string, requester: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.delete(`${apiUrl}/relationship/friends/request-reject`, {
      headers: { Authorization: auth },
  ...
  async getFriendRequestsPage(auth: string, page: number, page_size: number) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.get(`${apiUrl}/relationship/friends/requests/page/${page}/${page_size}`, {
      headers: { Authorization: auth }
    });
    return response.data;
  }
    ...
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.get(`${apiUrl}/relationship/friends/my-requests/page/${page}/${page_size}`, {
      headers: { Authorization: auth }
    });
    return response.data;
  }
  ...
  async getFriendsPage(auth: string, page: number, page_size: number) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.get(`${apiUrl}/relationship/friends/page/${page}/${page_size}`, {
      headers: { Authorization: auth }
    });
    return response.data;
  }
  ...
  async blockUser(auth: string, to_user_id: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.post(`${apiUrl}/relationship/blocks/block`, {
      to_user_id: to_user_id
    }, {
      headers: { Authorization: auth }
    });
  ...
  async unblockUser(auth: string, to_user_id: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.delete(`${apiUrl}/relationship/blocks/unblock`, {
      headers: { Authorization: auth },
      data: { to_user_id: to_user_id }
    });
    return response.data;
  ...
  async getFriendsSuggestionsPage(auth: string, page: number, page_size: number) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.get(`${apiUrl}/relationship/friends/suggestions/${page}/${page_size}`, {
      headers: { Authorization: auth }
    });
    return response.data;
  }
  ...
  async countMutualFriends(auth: string, other_user_id: string) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.get(`${apiUrl}/relationship/friends/mutual/count/${other_user_id}`, {
      headers: { Authorization: auth }
    });
    return response.data;
  }

  async getMutualFriendsPage(auth: string, other_user_id: string, page: number, page_size: number) {
    const apiUrl = process.env.DECODE_API_URL;
    const response = await axios.get(`${apiUrl}/relationship/friends/mutual/${other_user_id}/${page}/${page_size}`, {
      headers: { Authorization: auth }
    });
    return response.data;
  }
